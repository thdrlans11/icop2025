		...
		<div class="write-form-wrap">
			<form id="bbs_form" action="{{ route('board.upsert', ['code'=>$code, 'sid'=>!empty($data)?base64_encode($data['sid']):'']) }}" method="post" onsubmit="return check_bbs(this);" enctype="multipart/form-data">
				...
				<fieldset>
					...
					<ul class="write-wrap">
						<li>
							<div class="form-tit">Writer</div>
							<div class="form-con">
								<input type="text" name="name" id="name" value="{{ $data->name ?? config('site.common.info.name') }}" class="form-item small"/>
							</div>	
						</li>	
						{{-- <li>
						...
						@endif
						<li>
							<div class="form-tit">Subject</div>
							<div class="form-con">
								...
								<div class="checkbox-wrap cst mt-10">
									...
									<label for="main" class="checkbox-group">
										<input type="checkbox" name="main" id="main" value="Y" {{ ( $data->main ?? '' ) == 'Y' ? 'checked' : '' }}>Main
									</label>
									@endif
								</div>
							</div>
					...
					<div class="btn-wrap text-center">
						<a href="#n" class="btn btn-board btn-cancel" onclick="swalConfirm('취소하실 경우 작성하신 내역이 저장되지 않습니다.\n취소하시겠습니까?', '', function(){ location.href='{{ route('board.list', $code) }}' })">Cancel</a>
						<button type="submit" class="btn btn-board btn-write">{{ isset($data->sid) ? 'Modify' : 'OK' }}</button>
					</div>
				</fieldset>
			</form>
		</div>
